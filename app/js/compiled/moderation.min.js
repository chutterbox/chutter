...
})}helpers.each(data.datasets,function(dataset){var datasetObject={label:dataset.label||null,fillColor:dataset.fillColor,strokeColor:dataset.strokeColor,pointColor:dataset.pointColor,pointStrokeColor:dataset.pointStrokeColor,points:[]};this.datasets.push(datasetObject);helpers.each(dataset.data,function(dataPoint,index){var pointPosition;if(!this.scale.animation){pointPosition=this.scale.getPointPosition(index,this.scale.calculateCenterOffset(dataPoint))}datasetObject.points.push(new this.PointClass({value:dataPoint,label:data.labels[index],datasetLabel:dataset.label,x:this.options.animation?this.scale.xCenter:pointPosition.x,y:this.options.animation?this.scale.yCenter:pointPosition.y,strokeColor:dataset.pointStrokeColor,fillColor:dataset.pointColor,highlightFill:dataset.pointHighlightFill||dataset.pointColor,highlightStroke:dataset.pointHighlightStroke||dataset.pointStrokeColor}))},this)},this);this.render()},eachPoints:function(callback){helpers.each(this.datasets,function(dataset){helpers.each(dataset.points,callback,this)},this)},getPointsAtEvent:function(evt){var mousePosition=helpers.getRelativePosition(evt),fromCenter=helpers.getAngleFromPoint({x:this.scale.xCenter,y:this.scale.yCenter},mousePosition);var anglePerIndex=Math.PI*2/this.scale.valuesCount,pointIndex=Math.round((fromCenter.angle-Math.PI*1.5)/anglePerIndex),activePointsCollection=[];if(pointIndex>=this.scale.valuesCount||pointIndex<0){pointIndex=0}if(fromCenter.distance<=this.scale.drawingArea){helpers.each(this.datasets,function(dataset){activePointsCollection.push(dataset.points[pointIndex])})}return activePointsCollection},buildScale:function(data){this.scale=new Chart.RadialScale({display:this.options.showScale,fontStyle:this.options.scaleFontStyle,fontSize:this.options.scaleFontSize,fontFamily:this.options.scaleFontFamily,fontColor:this.options.scaleFontColor,showLabels:this.options.scaleShowLabels,showLabelBackdrop:this.options.scaleShowLabelBackdrop,backdropColor:this.options.scaleBackdropColor,backdropPaddingY:this.options.scaleBackdropPaddingY,backdropPaddingX:this.options.scaleBackdropPaddingX,lineWidth:this.options.scaleShowLine?this.options.scaleLineWidth:0,lineColor:this.options.scaleLineColor,angleLineColor:this.options.angleLineColor,angleLineWidth:this.options.angleShowLineOut?this.options.angleLineWidth:0,pointLabelFontColor:this.options.pointLabelFontColor,pointLabelFontSize:this.options.pointLabelFontSize,pointLabelFontFamily:this.options.pointLabelFontFamily,pointLabelFontStyle:this.options.pointLabelFontStyle,height:this.chart.height,width:this.chart.width,xCenter:this.chart.width/2,yCenter:this.chart.height/2,ctx:this.chart.ctx,templateString:this.options.scaleLabel,labels:data.labels,valuesCount:data.datasets[0].data.length});this.scale.setScaleSize();this.updateScaleRange(data.datasets);this.scale.buildYLabels()},updateScaleRange:function(datasets){var valuesArray=function(){var totalDataArray=[];helpers.each(datasets,function(dataset){if(dataset.data){totalDataArray=totalDataArray.concat(dataset.data)}else{helpers.each(dataset.points,function(point){totalDataArray.push(point.value)})}});return totalDataArray}();var scaleSizes=this.options.scaleOverride?{steps:this.options.scaleSteps,stepValue:this.options.scaleStepWidth,min:this.options.scaleStartValue,max:this.options.scaleStartValue+this.options.scaleSteps*this.options.scaleStepWidth}:helpers.calculateScaleRange(valuesArray,helpers.min([this.chart.width,this.chart.height])/2,this.options.scaleFontSize,this.options.scaleBeginAtZero,this.options.scaleIntegersOnly);helpers.extend(this.scale,scaleSizes)},addData:function(valuesArray,label){this.scale.valuesCount++;helpers.each(valuesArray,function(value,datasetIndex){var pointPosition=this.scale.getPointPosition(this.scale.valuesCount,this.scale.calculateCenterOffset(value));this.datasets[datasetIndex].points.push(new this.PointClass({value:value,label:label,x:pointPosition.x,y:pointPosition.y,strokeColor:this.datasets[datasetIndex].pointStrokeColor,fillColor:this.datasets[datasetIndex].pointColor}))},this);this.scale.labels.push(label);this.reflow();this.update()},removeData:function(){this.scale.valuesCount--;this.scale.labels.shift();helpers.each(this.datasets,function(dataset){dataset.points.shift()},this);this.reflow();this.update()},update:function(){this.eachPoints(function(point){point.save()});this.reflow();this.render()},reflow:function(){helpers.extend(this.scale,{width:this.chart.width,height:this.chart.height,size:helpers.min([this.chart.width,this.chart.height]),xCenter:this.chart.width/2,yCenter:this.chart.height/2});this.updateScaleRange(this.datasets);this.scale.setScaleSize();this.scale.buildYLabels()},draw:function(ease){var easeDecimal=ease||1,ctx=this.chart.ctx;this.clear();this.scale.draw();helpers.each(this.datasets,function(dataset){helpers.each(dataset.points,function(point,index){if(point.hasValue()){point.transition(this.scale.getPointPosition(index,this.scale.calculateCenterOffset(point.value)),easeDecimal)}},this);ctx.lineWidth=this.options.datasetStrokeWidth;ctx.strokeStyle=dataset.strokeColor;ctx.beginPath();helpers.each(dataset.points,function(point,index){if(index===0){ctx.moveTo(point.x,point.y)}else{ctx.lineTo(point.x,point.y)}},this);ctx.closePath();ctx.stroke();ctx.fillStyle=dataset.fillColor;ctx.fill();helpers.each(dataset.points,function(point){if(point.hasValue()){point.draw()}})},this)}})}).call(this);angular.module("templates-moderation",[]).run(["$templateCache",function($templateCache){$templateCache.put("../app/partials/moderation/dashboard.html",'<div layout=column flex=true layout-align="center center"><h3 class=md-title>Welcome to mod tools</h3><h6 class=md-subheader>Select a community you moderate from the "moderated communities" list in the toolbar to get started.</h6><md-button class=md-primary>Need help?</md-button></div>');$templateCache.put("../app/partials/moderation/index.html",'<!DOCTYPE html><html lang=en ng-app=ModerationApp xmlns:ng=http://angularjs.org><head><base href="/moderation/"><link rel=icon type=image/png href="favicon.ico"><link rel="shortcut icon" type=image/png href="favicon.ico"><meta name=fragment content="!"><meta charset="utf-8"><meta content="telephone=no" name="format-detection"><meta content="width=device-width,minimum-scale=1,maximum-scale=1,user-scalable=no" name="viewport"><meta content=ZYCtjzD0MQsstNXtDL7ZQyRzkj61deTZWYgEeMmCGdY name="google-site-verification"><link href="https://www.chutter.co/" rel="canonical"><link href=/vendor/angular-material/angular-material.css rel=stylesheet><link href=/vendor/angular-material-data-table/dist/md-data-table.css rel=stylesheet><link href=/vendor/angular-chart.js/dist/angular-chart.css rel=stylesheet><link href=/css/compiled/application.css rel=stylesheet><link href=/css/moderation.css rel=stylesheet><link href=/vendor/ionicons/css/ionicons.min.css rel=stylesheet><title>Chutter &mdash;</title><script>(function(i,s,o,g,r,a,m){i[\'GoogleAnalyticsObject\']=r;i[r]=i[r]||function(){  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)  })(window,document,\'script\',\'//www.google-analytics.com/analytics.js\',\'ga\');  ga(\'create\', \'UA-67283779-1\', \'auto\');  ga(\'send\', \'pageview\');</script><body layout=column flex=true md-theme={{theme}}><div ui-view=toolbar></div><md-content layout=row flex=true id=content><ui-view layout=column flex=true></ui-view><md-sidenav md-component-id=right md-is-locked-open="$mdMedia(\'gt-md\')" class="md-sidenav-right primary-content"><md-content ui-view=right-rail layout=column layout-fill=layout-fill></md-content><span flex=flex></span><footer-sidebar></footer-sidebar></md-sidenav></md-content></body><script src=/js/compiled/application.min.js></script><script src=/js/compiled/moderation.min.js></script><script src=//platform.twitter.com/widgets.js></script></head></html>');$templateCache.put("../app/partials/moderation/layout.html",'<chutter-toolbar></chutter-toolbar><md-content layout=row flex=flex><md-sidenav md-component-id=left md-is-locked-open="$mdMedia(\'gt-md\')" class=md-sidenav-left><md-content flex=flex role=navigation layout-fill=layout-fill layout=column class=nav-list><div ng-repeat="community in communities" layout=row class=nav-list-item><md-button ui-sref-active=active ui-sref="home.community.dashboard({id: community.slug})" layout=row layout-align="center start"><p ng-bind=::community.name></p></md-button></div></md-content></md-sidenav><div flex=flex layout=row class=md-whiteframe-z1 id=content><ui-view layout=column layout-fill=true></ui-view></div><md-sidenav md-component-id=right md-is-locked-open="$mdMedia(\'gt-md\')" class=md-sidenav-right><div layout=column ui-view=right-rail></div></md-sidenav></md-content>');$templateCache.put("../app/partials/moderation/toolbar.html",'<md-toolbar ng-class=page.mainToolbar md-scroll-shrink=true id=main-toolbar><div layout=row flex=100 layout-align="space-between center" class=md-toolbar-tools><div layout=row class=toolbar-continer><md-button style="background-color: transparent" href="/" layout=row layout-align="center center" id=logo><img ng-src="/img/logo.png"></md-button><a ng-bind=secondaryTitle id=secondary-header></a></div><div ng-hide="user &amp;&amp; user.id" layout=row layout-align="end end" class=toolbar-container id=auth-container><md-button aria-label="sign in" ng-click=logIn()>log in</md-button><md-button aria-label="sign in" ui-sref=register.welcome>sign up</md-button></div><div ng-show="user &amp;&amp; user.id" hide-sm=hide-sm hide-md=hide-md layout=row layout-align="end center" class=toolbar-container><md-button aria-label=moderation ng-if="user.moderator == \'true\'" href=/moderation target=_self>mod</md-button><md-button aria-label=management ng-if="user.manager == \'true\'" href=/management target=_self>management</md-button><md-button aria-label=notifications href="/me/" target=_self>{{user.username}}</md-button><div layout=row layout-align="start center" class=user-points>(<a href="/">{{user.post_points}}</a>·<a href="/">{{user.comment_points}}</a>)</div><md-button ng-class="{\'active\': unread_notifications}" aria-label=messages href="/me/" target=_self hide-sm=hide-sm layout=row class="md-icon-button icon-button-with-counter"><div><i class="icon ion-ios-bell"></i></div><div ng-if="unread_notifications &amp;&amp; unread_notifications &gt; 0"><span ng-bind=unread_notifications class=counter-text></span></div><md-tooltip>Notifications</md-tooltip></md-button><md-button ng-class="{\'active\': unread_messages}" aria-label=messages href=/me/conversations target=_self hide-sm=hide-sm layout=row class="md-icon-button icon-button-with-counter"><div><i class="icon ion-ios-chatboxes"></i></div><div ng-if="unread_messages &amp;&amp; unread_messages &gt; 0"><span ng-bind=unread_messages class=counter-text></span></div><md-tooltip>Conversations</md-tooltip></md-button><md-button aria-label=logout ng-click=logout() hide-sm=hide-sm class=md-icon-button><i class="icon ion-log-out"></i><md-tooltip>Log out</md-tooltip></md-button></div></div></md-toolbar><md-toolbar ng-class=page.mainToolbar id=networks-bar><div class=md-toolbar-tools><md-menu-bar id=network-menu-bar><md-menu><md-button ng-click=$mdOpenMenu()><span>Moderated Communities</span><md-icon><i class="icon ion-android-arrow-dropdown"></i></md-icon></md-button><md-menu-content><md-menu-item ng-repeat="community in communities"><md-button aria-label="moderate community" ui-sref="home.community({community: community.slug})">{{community.name}}</md-button></md-menu-item></md-menu-content></md-menu></md-menu-bar><md-tabs flex=true ng-class=page.mainToolbar class=md-primary><md-tab ui-sref=.dashboard><md-tab-label>Overview</md-tab-label></md-tab><md-tab ui-sref=.modwatch><md-tab-label>Modwatch</md-tab-label></md-tab><md-tab ui-sref=.inbox><md-tab-label>Inbox</md-tab-label></md-tab><md-tab ui-sref=.queue><md-tab-label>Queue</md-tab-label></md-tab><md-tab ui-sref=.policyGroups><md-tab-label>Policy Groups</md-tab-label></md-tab><md-tab ui-sref=.settings><md-tab-label>Settings</md-tab-label></md-tab><md-tab ui-sref=.rules><md-tab-label>Rules</md-tab-label></md-tab><md-tab ui-sref=.activityLog><md-tab-label>Activity Log</md-tab-label></md-tab><md-tab ui-sref=.moderators><md-tab-label>Moderators</md-tab-label></md-tab><md-tab ui-sref=.moderationRequests><md-tab-label>Moderator Requests</md-tab-label></md-tab></md-tabs></div></md-toolbar><md-progress-linear md-mode=indeterminate ng-if=loading class=md-hue-2 id=loading></md-progress-linear>');$templateCache.put("../app/partials/moderation/community/activity-log.html",'<md-data-table-toolbar><h2 class=md-title>Activity Log</h2></md-data-table-toolbar><md-data-table-container><table md-data-table=md-data-table><thead><tr><th name=User></th><th name=Action></th><th name=Context></th><th name=Item></th><th name="Rule Violation"></th></tr></thead><tbody ng-repeat="entry in activityLog.activity_log_entries" ng-switch=entry.entityable_type><td ng-bind=::entry.user.username></td><td ng-bind=::entry.action></td><td ng-bind=::entry.entityable_type></td><td ng-switch-when=Post ng-bind=::entry.entityable.title></td><td ng-switch-when=Comment ng-bind=::entry.entityable.body></td><td ng-switch-when=User ng-bind=::entry.entityable.username></td><td ng-bind=::entry.rule.category></td></tbody></table></md-data-table-container>');$templateCache.put("../app/partials/moderation/community/community.html",'<div layout=column layout-align="center center" flex=true><h3 class=md-title>Community Overvew (coming soon)</h3><h6 class=md-subheader>This will show traffic stats and general at-a-glance info about the community</h6></div>');$templateCache.put("../app/partials/moderation/community/inbox.html","<h1>hi 2</h1>");$templateCache.put("../app/partials/moderation/community/policy-groups.html","<div class=md-padding><h3>Banned Users</h3></div>");$templateCache.put("../app/partials/moderation/community/queue.html","");$templateCache.put("../app/partials/moderation/community/rules.html",'<div layout=column><md-list><md-list-item ng-repeat="rule in communityRules" class=md-3-line><div class=md-list-item-text><h3 ng-bind=rule.category></h3><h4 ng-bind=rule.detailed_explanation></h4><p ng-if=rule.general>Applies To: The General Community</p><p ng-if=rule.posts>Applies To: Posts</p><p ng-if=rule.comments>Applies To: Comments</p><p ng-if=rule.discouraged>Severity: Discouraged</p><p ng-if=rule.removal>Severity: Removal</p><p ng-if=rule.ban>Severity: Ban</p></div></md-list-item></md-list></div>');$templateCache.put("../app/partials/moderation/community/settings.html",'<md-content layout=column class=md-padding><form><div layout=row><md-input-container flex=50><label>Name</label><input ng-model="page.community.name"></md-input-container></div><div><md-input-container><label>Description</label><textarea rows=10 ng-model=page.community.description></textarea></md-input-container></div><div><md-input-container layout=row><md-checkbox ng-model=page.community.accepting_moderation_requests></md-checkbox><p>Accepting Moderation Requests</p></md-input-container></div><div><span flex=flex></span><md-button ng-click=update() class="md-primary md-raised">Save</md-button></div></form></md-content>')}]);(function(factory){"use strict";if(typeof define==="function"&&define.amd){define(["angular","chart.js"],factory)}else if(typeof exports==="object"){module.exports=factory(require("angular"),require("chart.js"))}else{factory(angular,Chart)}})(function(angular,Chart){"use strict";Chart.defaults.global.responsive=true;Chart.defaults.global.multiTooltipTemplate="<%if (datasetLabel){%><%=datasetLabel%>: <%}%><%= value %>";Chart.defaults.global.colours=["#97BBCD","#DCDCDC","#F7464A","#46BFBD","#FDB45C","#949FB1","#4D5360"];angular.module("chart.js",[]).provider("ChartJs",ChartJsProvider).factory("ChartJsFactory",["ChartJs",ChartJsFactory]).directive("chartBase",["ChartJsFactory",function(ChartJsFactory){return new ChartJsFactory}]).directive("chartLine",["ChartJsFactory",function(ChartJsFactory){return new ChartJsFactory("Line")}]).directive("chartBar",["ChartJsFactory",function(ChartJsFactory){return new ChartJsFactory("Bar")}]).directive("chartRadar",["ChartJsFactory",function(ChartJsFactory){return new ChartJsFactory("Radar")}]).directive("chartDoughnut",["ChartJsFactory",function(ChartJsFactory){return new ChartJsFactory("Doughnut")}]).directive("chartPie",["ChartJsFactory",function(ChartJsFactory){return new ChartJsFactory("Pie")}]).directive("chartPolarArea",["ChartJsFactory",function(ChartJsFactory){return new ChartJsFactory("PolarArea")}]);function ChartJsProvider(){var options={};var ChartJs={Chart:Chart,getOptions:function(type){var typeOptions=type&&options[type]||{};return angular.extend({},options,typeOptions)}};this.setOptions=function(type,customOptions){if(!customOptions){customOptions=type;options=angular.extend(options,customOptions);return}options[type]=angular.extend(options[type]||{},customOptions)};this.$get=function(){return ChartJs}}function ChartJsFactory(ChartJs){return function chart(type){return{restrict:"CA",scope:{data:"=",labels:"=",options:"=",series:"=",colours:"=?",getColour:"=?",chartType:"=",legend:"@",click:"=",hover:"="},link:function(scope,elem){var chart,container=document.createElement("div");container.className="chart-container";elem.replaceWith(container);container.appendChild(elem[0]);if(typeof window.G_vmlCanvasManager==="object"&&window.G_vmlCanvasManager!==null){if(typeof window.G_vmlCanvasManager.initElement==="function"){window.G_vmlCanvasManager.initElement(elem[0])}}scope.$watch("data",function(newVal,oldVal){if(!newVal||!newVal.length||Array.isArray(newVal[0])&&!newVal[0].length)return;var chartType=type||scope.chartType;if(!chartType)return;if(chart){if(canUpdateChart(newVal,oldVal))return updateChart(chart,newVal,scope);chart.destroy()}chart=createChart(chartType,scope,elem)},true);scope.$watch("series",resetChart,true);scope.$watch("labels",resetChart,true);scope.$watch("options",resetChart,true);scope.$watch("colours",resetChart,true);scope.$watch("chartType",function(newVal,oldVal){if(isEmpty(newVal))return;if(angular.equals(newVal,oldVal))return;if(chart)chart.destroy();chart=createChart(newVal,scope,elem)});scope.$on("$destroy",function(){if(chart)chart.destroy()});function resetChart(newVal,oldVal){if(isEmpty(newVal))return;if(angular.equals(newVal,oldVal))return;var chartType=type||scope.chartType;if(!chartType)return;if(chart)chart.destroy();chart=createChart(chartType,scope,elem)}}}};function canUpdateChart(newVal,oldVal){if(newVal&&oldVal&&newVal.length&&oldVal.length){return Array.isArray(newVal[0])?newVal.length===oldVal.length&&newVal[0].length===oldVal[0].length:oldVal.reduce(sum,0)>0?newVal.length===oldVal.length:false}return false}function sum(carry,val){return carry+val}function createChart(type,scope,elem){if(!scope.data||!scope.data.length)return;scope.getColour=typeof scope.getColour==="function"?scope.getColour:getRandomColour;scope.colours=getColours(type,scope);var cvs=elem[0],ctx=cvs.getContext("2d");var data=Array.isArray(scope.data[0])?getDataSets(scope.labels,scope.data,scope.series||[],scope.colours):getData(scope.labels,scope.data,scope.colours);var options=angular.extend({},ChartJs.getOptions(type),scope.options);var chart=new ChartJs.Chart(ctx)[type](data,options);scope.$emit("create",chart);["hover","click"].forEach(function(action){if(scope[action])cvs[action==="click"?"onclick":"onmousemove"]=getEventHandler(scope,chart,action)});if(scope.legend&&scope.legend!=="false")setLegend(elem,chart);return chart}function getEventHandler(scope,chart,action){return function(evt){var atEvent=chart.getPointsAtEvent||chart.getBarsAtEvent||chart.getSegmentsAtEvent;if(atEvent){var activePoints=atEvent.call(chart,evt);scope[action](activePoints,evt);scope.$apply()}}}function getColours(type,scope){var colours=angular.copy(scope.colours||ChartJs.getOptions(type).colours||Chart.defaults.global.colours);while(colours.length<scope.data.length){colours.push(scope.getColour())}return colours.map(convertColour)}function convertColour(colour){if(typeof colour==="object"&&colour!==null)return colour;if(typeof colour==="string"&&colour[0]==="#")return getColour(hexToRgb(colour.substr(1)));return getRandomColour()}function getRandomColour(){var colour=[getRandomInt(0,255),getRandomInt(0,255),getRandomInt(0,255)];return getColour(colour)}function getColour(colour){return{fillColor:rgba(colour,.2),strokeColor:rgba(colour,1),pointColor:rgba(colour,1),pointStrokeColor:"#fff",pointHighlightFill:"#fff",pointHighlightStroke:rgba(colour,.8)}}function getRandomInt(min,max){return Math.floor(Math.random()*(max-min+1))+min}function rgba(colour,alpha){return"rgba("+colour.concat(alpha).join(",")+")"}function hexToRgb(hex){var bigint=parseInt(hex,16),r=bigint>>16&255,g=bigint>>8&255,b=bigint&255;return[r,g,b]}function getDataSets(labels,data,series,colours){return{labels:labels,datasets:data.map(function(item,i){return angular.extend({},colours[i],{label:series[i],data:item})})}}function getData(labels,data,colours){return labels.map(function(label,i){return angular.extend({},colours[i],{label:label,value:data[i],color:colours[i].strokeColor,highlight:colours[i].pointHighlightStroke})})}function setLegend(elem,chart){var $parent=elem.parent(),$oldLegend=$parent.find("chart-legend"),legend="<chart-legend>"+chart.generateLegend()+"</chart-legend>";if($oldLegend.length)$oldLegend.replaceWith(legend);else $parent.append(legend)}function updateChart(chart,values,scope){if(Array.isArray(scope.data[0])){chart.datasets.forEach(function(dataset,i){(dataset.points||dataset.bars).forEach(function(dataItem,j){dataItem.value=values[i][j]})})}else{chart.segments.forEach(function(segment,i){segment.value=values[i]})}chart.update();scope.$emit("update",chart)}function isEmpty(value){return!value||Array.isArray(value)&&!value.length||typeof value==="object"&&!Object.keys(value).length}}});angular.module("md.data.table",["md.table.templates"]);angular.module("md.data.table").directive("mdColumnHeader",mdColumnHeader);function mdColumnHeader($compile,$interpolate,$timeout){"use strict";function postLink(scope,element,attrs,ctrls){var tableCtrl=ctrls[0];var headCtrl=ctrls[1];var template=angular.element("<div></div>");template.text($interpolate.startSymbol()+"name"+$interpolate.endSymbol());if(attrs.unit){template.text(template.text()+" ("+$interpolate.startSymbol()+"unit"+$interpolate.endSymbol()+")")}if(angular.isDefined(attrs.trim)){template.contents().wrap("<div></div>")}if(attrs.orderBy){var sortIcon=angular.element("<md-icon></md-icon>");var isActive=function(){return headCtrl.order===scope.order||headCtrl.order==="-"+scope.order};var setOrder=function(){if(isActive()){scope.$apply(headCtrl.order=headCtrl.order===scope.order?"-"+scope.order:scope.order)}else{scope.$apply(headCtrl.order=angular.isDefined(attrs.descendFirst)?"-"+scope.order:scope.order)}if(headCtrl.pullTrigger){$timeout(headCtrl.pullTrigger)}};scope.getDirection=function(){if(isActive()){return headCtrl.order[0]==="-"?"down":"up"}return angular.isDefined(attrs.descendFirst)?"down":"up"};sortIcon.attr("md-svg-icon","templates.arrow.html");sortIcon.attr("ng-class","getDirection()");if(angular.isDefined(attrs.numeric)){template.prepend(sortIcon)}else{template.append(sortIcon)}element.on("click",setOrder);scope.$watch(isActive,function(active){if(active){element.addClass("md-active")}else{element.removeClass("md-active")}})}element.append($compile(template)(scope));if(headCtrl.isSignificant(element.parent())){tableCtrl.setColumn(attrs);if(attrs.ngRepeat){if(scope.$parent.$last){tableCtrl.isReady.head.resolve()}}else if(tableCtrl.isLastChild(element.parent().children(),element[0])){tableCtrl.isReady.head.resolve()}}}return{link:postLink,require:["^^mdDataTable","^mdTableHead"],scope:{name:"@",order:"@orderBy",unit:"@"}}}mdColumnHeader.$inject=["$compile","$interpolate","$timeout"];angular.module("md.data.table").directive("mdDataTable",mdDataTable);function mdDataTable(){"use strict";function compile(tElement,tAttrs){var head=tElement.find("thead");var foot=tElement.find("tfoot");if(!head.length){head=tElement.find("tbody").eq(0);if(head.children().find("th").length){head.replaceWith("<thead>"+head.html()+"</thead>")}else{throw new Error("mdDataTable","Expecting <thead></thead> element.")}head=tElement.find("thead")}var rows=tElement.find("tbody").find("tr");head.attr("md-table-head","");rows.attr("md-table-row","");rows.find("td").attr("md-table-cell","");if(foot.length){foot.attr("md-table-foot","");if(tAttrs.mdRowSelect){foot.find("tr").prepend("<td></td>")}}if(tAttrs.mdRowSelect&&rows.attr("ng-repeat")){rows.attr("md-select-row","")}if(tAttrs.mdRowSelect&&!rows.attr("ng-repeat")){console.warn("Please use ngRepeat to enable row selection.")}if(head.attr("md-order")&&!rows.attr("ng-repeat")){console.warn("Column ordering without ngRepeat is not supported.")}}function Controller($attrs,$element,$q,$scope){var self=this;self.columns=[];self.classes=[];self.isReady={body:$q.defer(),head:$q.defer()};if($attrs.mdRowSelect){self.columns.push({isNumeric:false});if(!angular.isArray(self.selectedItems)){self.selectedItems=[];console.warn('md-row-select="'+$attrs.mdRowSelect+'" : '+$attrs.mdRowSelect+" is not defined as an array in your controller, "+"i.e. "+$attrs.mdRowSelect+" = [], two-way data binding will fail.")}}if($attrs.mdProgress){$scope.$watch("$mdDataTableCtrl.progress",function(){var deferred=self.defer();$q.when(self.progress)["finally"](deferred.resolve)})}["md-primary","md-hue-1","md-hue-2","md-hue-3"].forEach(function(mdClass){if($element.hasClass(mdClass)){self.classes.push(mdClass)}});self.defer=function(){if(self.deferred){self.deferred.reject("cancel")}else if(self.showProgress){self.showProgress()}self.deferred=$q.defer();self.deferred.promise.then(self.resolve);return self.deferred};self.resolve=function(){self.deferred=undefined;if(self.hideProgress){self.hideProgress()}};self.isLastChild=function(siblings,child){return Array.prototype.indexOf.call(siblings,child)===siblings.length-1};self.isReady.body.promise.then(function(ngRepeat){if($attrs.mdRowSelect&&ngRepeat){self.listener=$scope.$parent.$watch(ngRepeat.items,function(newValue,oldeValue){if(newValue!==oldeValue){self.selectedItems.splice(0)}})}});self.setColumn=function(column){self.columns.push({isNumeric:angular.isDefined(column.numeric),unit:column.unit})}}Controller.$inject=["$attrs","$element","$q","$scope"];return{bindToController:{progress:"=mdProgress",selectedItems:"=mdRowSelect"},compile:compile,controller:Controller,controllerAs:"$mdDataTableCtrl",restrict:"A",scope:{}}}angular.module("md.data.table").directive("mdTableCell",mdTableCell);function mdTableCell(){"use strict";function postLink(scope,element){var select=element.find("md-select");if(select.length){select.on("click",function(event){event.stopPropagation()});element.addClass("clickable").on("click",function(event){event.stopPropagation();select[0].click()})}}function compile(tElement){tElement.find("md-select").attr("md-container-class","md-table-select");return postLink}return{compile:compile}}angular.module("md.data.table").directive("mdTableFoot",mdTableFoot);function mdTableFoot(){"use strict";function postLink(scope,element,attrs,tableCtrl){var cells=element.find("td");tableCtrl.columns.forEach(function(column,index){if(column.isNumeric){cells.eq(index).addClass("numeric")}});if(cells.length<tableCtrl.columns.length){element.find("tr").append('<td colspan="'+(tableCtrl.columns.length-cells.length)+'"></td>')}}return{require:"^mdDataTable",link:postLink}}angular.module("md.data.table").directive("mdTableHead",mdTableHead);function mdTableHead($mdTable,$q){"use strict";function compile(tElement){tElement.find("th").attr("md-column-header","");if(tElement.parent().attr("md-row-select")){var ngRepeat=tElement.parent().find("tbody").find("tr").attr("ng-repeat");if(ngRepeat){tElement.find("tr").prepend(angular.element('<th md-select-all="'+$mdTable.parse(ngRepeat).items+'"></th>'))}}tElement.after("<thead md-table-progress></thead>");return postLink}function Controller($element,$scope){var rows=$element.find("tr");if(!$scope.sigRow||parseInt($scope.sigRow,10)===isNaN()||$scope.sigRow<0){$scope.sigRow=rows.length-1}this.isSignificant=function(row){return row.prop("rowIndex")===$scope.sigRow}}function postLink(scope,element,attrs,tableCtrl){var controller=element.data("$mdTableHeadController");if(angular.isFunction(scope.trigger)){controller.pullTrigger=function(){var deferred=tableCtrl.defer();$q.when(scope.trigger(controller.order))["finally"](deferred.resolve)}}}Controller.$inject=["$element","$scope"];return{bindToController:{order:"=mdOrder"},compile:compile,controller:Controller,controllerAs:"$mdDataTableHeadCtrl",require:"^mdDataTable",scope:{trigger:"=?mdTrigger",sigRow:"=?"}}}mdTableHead.$inject=["$mdTable","$q"];angular.module("md.data.table").directive("mdDataTablePagination",mdDataTablePagination);function mdDataTablePagination($q){"use strict";function postLink(scope,element,attrs){scope.paginationLabel={text:"Rows per page:",of:"of"};if(angular.isObject(scope.label)){angular.extend(scope.paginationLabel,scope.label)}if(angular.isFunction(scope.trigger)){var findTable=function(element,callback){while(element.localName!=="md-data-table-container"&&element.previousElementSibling){element=element.previousElementSibling}callback(angular.element(element.firstElementChild))};var setTrigger=function(table){var tableCtrl=table.controller("mdDataTable");if(!tableCtrl){return console.warn("Table Pagination: Could not locate your table directive, your "+attrs.mdTrigger+" function will not work.")}scope.pullTrigger=function(){var deferred=tableCtrl.defer();$q.when(scope.trigger(scope.page,scope.limit))["finally"](deferred.resolve)}};findTable(element.prop("previousElementSibling"),setTrigger)}}function Controller($scope,$timeout){var min=1;$scope.hasNext=function(){return $scope.page*$scope.limit<$scope.total};$scope.hasPrevious=function(){return $scope.page>1};$scope.next=function(){$scope.page++;if($scope.pullTrigger){$timeout($scope.pullTrigger)}min=$scope.min()};$scope.last=function(){$scope.page=Math.ceil($scope.total/$scope.limit);if($scope.pullTrigger){$timeout($scope.pullTrigger)}min=$scope.min()};$scope.min=function(){return($scope.page-1)*$scope.limit+1};$scope.max=function(){return $scope.hasNext()?$scope.page*$scope.limit:$scope.total};$scope.onSelect=function(){$scope.page=Math.floor(min/$scope.limit)+1;if($scope.pullTrigger){$timeout($scope.pullTrigger)}min=$scope.min();while(min>$scope.total&&$scope.hasPrevious()){$scope.previous()}};$scope.previous=function(){$scope.page--;if($scope.pullTrigger){$timeout($scope.pullTrigger)}min=$scope.min()};$scope.first=function(){$scope.page=1;if($scope.pullTrigger){$timeout($scope.pullTrigger)}min=$scope.min()}}Controller.$inject=["$scope","$timeout"];return{controller:Controller,scope:{label:"=mdLabel",limit:"=mdLimit",page:"=mdPage",rowSelect:"=mdRowSelect",total:"@mdTotal",trigger:"=mdTrigger"},templateUrl:"templates.md-data-table-pagination.html",link:postLink}}mdDataTablePagination.$inject=["$q"];angular.module("md.data.table").directive("mdTableProgress",mdTableProgress);function mdTableProgress(){"use strict";function postLink(scope,element,attrs,tableCtrl){scope.getColumnCount=function(){return tableCtrl.columns.length};tableCtrl.hideProgress=function(){scope.showProgress=false};tableCtrl.showProgress=function(){scope.showProgress=true}}return{link:postLink,require:"^mdDataTable",replace:true,templateUrl:"templates.md-data-table-progress.html"}}angular.module("md.data.table").directive("mdTableRow",mdTableRow);function mdTableRow($mdTable,$timeout){"use strict";function postLink(scope,element,attrs,tableCtrl){if(angular.isDefined(attrs.mdSelectRow)){
scope.mdClasses=tableCtrl.classes;scope.isDisabled=function(){return scope.$eval(attrs.mdDisableSelect)};scope.isSelected=function(item){return tableCtrl.selectedItems.indexOf(item)!==-1};scope.toggleRow=function(item,event){event.stopPropagation();if(scope.isDisabled()){return}if(scope.isSelected(item)){tableCtrl.selectedItems.splice(tableCtrl.selectedItems.indexOf(item),1)}else{tableCtrl.selectedItems.push(item)}}}if(attrs.ngRepeat){if(scope.$last){tableCtrl.isReady.body.resolve($mdTable.parse(attrs.ngRepeat))}}else if(tableCtrl.isLastChild(element.parent().children(),element[0])){tableCtrl.isReady.body.resolve()}tableCtrl.isReady.head.promise.then(function(){tableCtrl.columns.forEach(function(column,index){if(column.isNumeric){var cell=element.children().eq(index);cell.addClass("numeric");if(angular.isDefined(cell.attr("show-unit"))){$timeout(function(){cell.text(cell.text()+tableCtrl.columns[index].unit)})}}})})}return{link:postLink,require:"^^mdDataTable"}}mdTableRow.$inject=["$mdTable","$timeout"];angular.module("md.data.table").factory("$mdTable",mdTableService);function mdTableService(){"use strict";var cache={};function Repeat(ngRepeat){this._tokens=ngRepeat.split(" ");this._iterator=0;this.item=this.current();while(this.hasNext()&&this.getNext()!=="in"){this.item+=this.current()}this.items=this.getNext();while(this.hasNext()&&["|","track"].indexOf(this.getNext())===-1){this.items+=this.current()}}Repeat.prototype.current=function(){return this._tokens[this._iterator]};Repeat.prototype.getNext=function(){return this._tokens[++this._iterator]};Repeat.prototype.getValue=function(){return this._tokens.join(" ")};Repeat.prototype.hasNext=function(){return this._iterator<this._tokens.length-1};function parse(ngRepeat){if(!cache.hasOwnProperty(ngRepeat)){return cache[ngRepeat]=new Repeat(ngRepeat)}return cache[ngRepeat]}return{parse:parse}}angular.module("md.data.table").directive("mdSelectAll",mdSelectAll);function mdSelectAll(){"use strict";function template(tElement){var checkbox=angular.element("<md-checkbox></md-checkbox>");checkbox.attr("aria-label","Select All");checkbox.attr("ng-click","toggleAll()");checkbox.attr("ng-class","mdClasses");checkbox.attr("ng-checked","allSelected()");checkbox.attr("ng-disabled","!getCount()");tElement.append(checkbox)}function postLink(scope,element,attrs,tableCtrl){var count=0;var getSelectableItems=function(){return scope.items.filter(function(item){return!tableCtrl.isDisabled(item)})};tableCtrl.isReady.body.promise.then(function(){scope.mdClasses=tableCtrl.classes;scope.getCount=function(){return count=scope.items.reduce(function(sum,item){return tableCtrl.isDisabled(item)?sum:++sum},0)};scope.allSelected=function(){return count&&count===tableCtrl.selectedItems.length};scope.toggleAll=function(){var selectableItems=getSelectableItems(scope.items);if(selectableItems.length===tableCtrl.selectedItems.length){tableCtrl.selectedItems.splice(0)}else{tableCtrl.selectedItems=selectableItems}}})}return{link:postLink,require:"^^mdDataTable",scope:{items:"=mdSelectAll"},template:template}}angular.module("md.data.table").directive("mdSelectRow",mdSelectRow);function mdSelectRow($mdTable){"use strict";function template(tElement,tAttrs){var ngRepeat=$mdTable.parse(tAttrs.ngRepeat);var checkbox=angular.element("<md-checkbox></md-checkbox>");checkbox.attr("aria-label","Select Row");checkbox.attr("ng-click","toggleRow("+ngRepeat.item+", $event)");checkbox.attr("ng-class","mdClasses");checkbox.attr("ng-checked","isSelected("+ngRepeat.item+")");if(tAttrs.mdDisableSelect){checkbox.attr("ng-disabled","isDisabled()")}tElement.prepend(angular.element("<td></td>").append(checkbox));if(angular.isDefined(tAttrs.mdAutoSelect)){tAttrs.$set("ngClick","toggleRow("+ngRepeat.item+", $event)")}tAttrs.$set("ngClass","{'md-selected': isSelected("+ngRepeat.item+")}")}function postLink(scope,element,attrs,tableCtrl){var model={};var ngRepeat=$mdTable.parse(attrs.ngRepeat);if(!angular.isFunction(scope.isDisabled)){scope.isDisabled=function(){return false}}tableCtrl.isDisabled=function(item){model[ngRepeat.item]=item;return scope.isDisabled(model)}}return{link:postLink,priority:1001,require:"^^mdDataTable",scope:{isDisabled:"&?mdDisableSelect"},template:template}}mdSelectRow.$inject=["$mdTable"];angular.module("md.table.templates",["templates.arrow.html","templates.navigate-before.html","templates.navigate-first.html","templates.navigate-last.html","templates.navigate-next.html","templates.md-data-table-pagination.html","templates.md-data-table-progress.html"]);angular.module("templates.arrow.html",[]).run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates.arrow.html",'<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18"><path d="M3,9 L4.06,10.06 L8.25,5.87 L8.25,15 L9.75,15 L9.75,5.87 L13.94,10.06 L15,9 L9,3 L3,9 L3,9 Z"/></svg>')}]);angular.module("templates.navigate-before.html",[]).run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates.navigate-before.html",'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg>\n'+"")}]);angular.module("templates.navigate-first.html",[]).run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates.navigate-first.html",'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M7 6 v12 h2 v-12 h-2z M17.41 7.41L16 6l-6 6 6 6 1.41-1.41L12.83 12z"/></svg>\n'+"")}]);angular.module("templates.navigate-last.html",[]).run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates.navigate-last.html",'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M15 6 v12 h2 v-12 h-2z M8 6L6.59 7.41 11.17 12l-4.58 4.59L8 18l6-6z"/></svg>\n'+"")}]);angular.module("templates.navigate-next.html",[]).run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates.navigate-next.html",'<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>\n'+"")}]);angular.module("templates.md-data-table-pagination.html",[]).run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates.md-data-table-pagination.html","<div>\n"+'  <span class="label">{{paginationLabel.text}}</span>\n'+'  <md-select ng-model="limit" md-container-class="md-pagination-select" ng-change="onSelect()" aria-label="Row Count" placeholder="{{rowSelect ? rowSelect[0] : 5}}">\n'+'    <md-option ng-repeat="rows in rowSelect ? rowSelect : [5, 10, 15]" ng-value="rows">{{rows}}</md-option>\n'+"  </md-select>\n"+"  <span>{{min()}} - {{max()}} {{paginationLabel.of}} {{total}}</span>\n"+"</div>\n"+"<div>\n"+'  <md-button type="button" ng-click="first()" ng-disabled="!hasPrevious()" aria-label="First">\n'+'    <md-icon md-svg-icon="templates.navigate-first.html"></md-icon>\n'+"  </md-button>\n"+'  <md-button type="button" ng-click="previous()" ng-disabled="!hasPrevious()" aria-label="Previous">\n'+'    <md-icon md-svg-icon="templates.navigate-before.html"></md-icon>\n'+"  </md-button>\n"+'  <md-button type="button" ng-click="next()" ng-disabled="!hasNext()" aria-label="Next">\n'+'    <md-icon md-svg-icon="templates.navigate-next.html"></md-icon>\n'+"  </md-button>\n"+'  <md-button type="button" ng-click="last()" ng-disabled="!hasNext()" aria-label="Last">\n'+'    <md-icon md-svg-icon="templates.navigate-last.html"></md-icon>\n'+"  </md-button>\n"+"</div>\n"+"")}]);angular.module("templates.md-data-table-progress.html",[]).run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates.md-data-table-progress.html",'<thead ng-if="showProgress">\n'+"  <tr>\n"+'    <th colspan="{{getColumnCount()}}">\n'+'      <md-progress-linear md-mode="indeterminate"></md-progress-linear>\n'+"    </th>\n"+"  </tr>\n"+"</thead>")}]);(function(){var app;app=angular.module("ModerationApp",["Chutter","chart.js","md.data.table","templates-moderation"]);app.config(["$mdThemingProvider",function($mdThemingProvider){return $mdThemingProvider.theme("default").primaryPalette("indigo")}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("dashboardCtrl",function(){})}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("homeCtrl",["$scope","Communities","Page",function($scope,Communities,Page){$scope.communities=Communities;return $scope.page=Page}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("toolbarCtrl",["$auth","$scope","$mdDialog","poller","$mdToast","API","Communities","$state","$location",function($auth,$scope,$mdDialog,poller,$mdToast,API,Communities,$state,$location){$scope.communities=Communities;$scope.notifications=[];$scope.$on("$stateChangeStart",function(){return $scope.loading=true});$scope.$on("$stateChangeSuccess",function(){return $scope.loading=false});$scope.applicationSectionNamespace="frontpage";$scope.$on("auth:show-signin",function(){return $scope.logIn});$scope.logIn=function(){$mdDialog.show({controller:"authCtrl",templateUrl:"../app/partials/main/authenticate.html",parent:angular.element(document.body),clickOutsideToClose:true});$scope.startPollers=function(){poller.get(API.makeURL("/users/ephemeral_notifications"),{delay:3e4}).promise.then(null,null,function(data){return $scope.showNotifications(data.data)});poller.get(API.makeURL("/users/toolbar_notifications"),{delay:3e4}).promise.then(null,null,function(resp){$scope.unread_messages=resp.data.unread_messages;return $scope.unread_notifications=resp.data.unread_notifications});return $scope.poller=poller};$scope.logout=function(){return $auth.signOut().then(function(){return $location.url("/")})};_.defer(function(){if($scope.user&&$scope.user.id){return $scope.startPollers()}});$scope.$on("auth:logout-success",function(){return $scope.poller.stopAll()});return $scope.$on("auth:login-success",function(){return $scope.startPollers()})};$scope.showNotifications=function(notifications){var cascade,ephemeral_notifications;ephemeral_notifications=_.reject(notifications,function(n){return n.ephemeral_count===0});if(ephemeral_notifications.length>0){if(ephemeral_notifications.length>1){cascade=true}return _.each(ephemeral_notifications,function(notification){if(cascade){return setTimeout(function(){return $scope.showNotification(notification)},5e3)}else{return $scope.showNotification(notification)}})}};return $scope.showNotification=function(notification){var body,title;if(notification.entityable==="comment"){body=notification.body.substring(0,50);if(body.length>49){body+="..."}return $mdToast.show($mdToast.simple().content("Re: "+body).position("bottom right").action(notification.ephemeral_count+" new").hideDelay(5e3))}else if(notification.entityable==="post"){title=notification.title.substring(0,50);if(title.length>49){title+="..."}return $mdToast.show($mdToast.simple().content("Re: "+title).position("bottom right").action(notification.ephemeral_count+" new").hideDelay(5e3))}}}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("communityCtrl",["$scope","Page",function($scope,Page){return $scope.page=Page}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("communityDashboardCtrl",["$scope",function($scope){$scope.labels=["today","yesterday","yesterday"];$scope.data=[[100,200,323,435,60,70,80,90,100],[100,20,323,435,60,70,8,90,100],[100,200,33,435,60,70,8,90,100]];$scope.series=["Series A","Series B"];$scope.options={pointDot:true,datasetStrokeWidth:.2};return $scope.onClick=function(points,evt){return console.log(points,evt)}}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("InboxCtrl",["$scope",function($scope){$scope.labels=["today","yesterday","yesterday"];$scope.data=[[100,200,323,435,60,70,80,90,100],[100,20,323,435,60,70,8,90,100],[100,200,33,435,60,70,8,90,100]];$scope.series=["Series A","Series B"];$scope.options={pointDot:true,datasetStrokeWidth:.2};return $scope.onClick=function(points,evt){return console.log(points,evt)}}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("moderatorListCtrl",["$scope","Moderators",function($scope,Moderators){return $scope.moderators=Moderators}]);app.controller("moderationRequestListCtrl",["$scope","ModerationRequests",function($scope,ModerationRequests){return $scope.moderationRequests=ModerationRequests}]);app.controller("editModeratorCtrl",["$scope","Moderator","CommunityResource","$stateParams",function($scope,Moderator,CommunityResource,$stateParams){$scope.moderator=Moderator;return $scope.updateModerator=function(){return CommunityResource.updateModerator({id:$stateParams.id,community_subscription:$scope.moderator.community_subscription})}}]);app.controller("editModerationRequestCtrl",["$scope","CommunityResource","$stateParams","UserStats","$state",function($scope,CommunityResource,$stateParams,UserStats,$state){$scope.addModerator=function(){return CommunityResource.addModerator({id:$stateParams.id,user_id:$stateParams.user_id}).$promise.then(function(data){console.log(data.status);return $state.transitionTo("home.community.moderators.edit",{id:$stateParams.id,user_id:$stateParams.user_id})})};return $scope.dismissModerationPositionRequest=function(){return CommunityResource.dismissModerationPositionRequest({id:$stateParams.id,user_id:$stateParams.user_id}).$promise.then(function(data){return $state.transitionTo("^")})}}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("communityNotificationsCtrl",["$scope",function($scope){$scope.labels=["today","yesterday","yesterday"];$scope.data=[[100,200,323,435,60,70,80,90,100],[100,20,323,435,60,70,8,90,100],[100,200,33,435,60,70,8,90,100]];$scope.series=["Series A","Series B"];$scope.options={pointDot:true,datasetStrokeWidth:.2};return $scope.onClick=function(points,evt){return console.log(points,evt)}}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("queueListCtrl",["$scope","ReportedItems",function($scope,ReportedItems){return $scope.reportedItems=ReportedItems}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("communitySettingsCtrl",["$scope","Page","CommunityResource",function($scope,Page,CommunityResource){$scope.page=Page;return $scope.update=function(){return CommunityResource.update({id:$scope.page.community.id,community:$scope.page.community})}}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("InboxCtrl",["$scope",function($scope){$scope.labels=["today","yesterday","yesterday"];$scope.data=[[100,200,323,435,60,70,80,90,100],[100,20,323,435,60,70,8,90,100],[100,200,33,435,60,70,8,90,100]];$scope.series=["Series A","Series B"];$scope.options={pointDot:true,datasetStrokeWidth:.2};return $scope.onClick=function(points,evt){return console.log(points,evt)}}])}).call(this);(function(){}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("PolicyGroupsCtrl",["$scope",function($scope){$scope.labels=["today","yesterday","yesterday"];$scope.data=[[100,200,323,435,60,70,80,90,100],[100,20,323,435,60,70,8,90,100],[100,200,33,435,60,70,8,90,100]];$scope.series=["Series A","Series B"];$scope.options={pointDot:true,datasetStrokeWidth:.2};return $scope.onClick=function(points,evt){return console.log(points,evt)}}])}).call(this);(function(){var app;app=angular.module("ModerationApp");app.controller("rulesCtrl",["$scope","communityRules",function($scope,communityRules){return $scope.communityRules=communityRules}])}).call(this);(function(){}).call(this);(function(){var app;app=angular.module("ModerationApp");app.factory("UserResource",["$resource","API",function($resource,API){return $resource(API.makeURL("/users/"),{id:"@id"},{moderatedCommunities:{url:API.makeURL("/users/moderated_communities"),method:"GET",isArray:true},stats:{url:API.makeURL("/users/:id/stats"),isArray:true}})}])}).call(this);(function(){"use strict";var app;app=angular.module("ModerationApp");app.config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){var activityLog,community,communityInbox,communityQueue,home,moderationRequestEdit,moderationRequests,moderatorEdit,moderators,modwatch,policyGroups,rules,settings,view_url;view_url="../app/partials/moderation";home={name:"home",url:"/",resolve:{Communities:["UserResource",function(UserResource){return UserResource.moderatedCommunities().$promise}]},views:{toolbar:{templateUrl:view_url+"/toolbar.html",controller:"toolbarCtrl"},"":{templateUrl:view_url+"/dashboard.html"}}};community={name:"home.community",url:"c/:community",views:{"@":{templateUrl:view_url+"/community/community.html",resolve:{Community:["CommunityResource","$stateParams",function(CommunityResource,$stateParams){return CommunityResource.show({id:$stateParams.community}).$promise}]}}}};communityInbox={name:"home.community.inbox",url:"/inbox",templateUrl:view_url+"/community/inbox.html"};communityQueue={name:"home.community.queue",url:"/queue",views:{"right-rail@home":{templateUrl:view_url+"/community/queue/queueList.html",resolve:{ReportedItems:["CommunityResource","$stateParams",function(CommunityResource,$stateParams){return CommunityResource.reportedItems({id:$stateParams.id})}]},controller:"queueListCtrl"}}};modwatch={name:"home.community.modwatch",url:"/modwatch",templateUrl:view_url+"/community/modwatch.html",resolve:{Modwatch:["CommunityResource","$stateParams",function(CommunityResource,$stateParams){return CommunityResource.modwatch({id:$stateParams.id}).$promise}]}};policyGroups={name:"home.community.policyGroups",url:"/policy-groups",templateUrl:view_url+"/community/policy-groups.html",resolve:{BanList:["CommunityResource","$stateParams",function(CommunityResource,$stateParams){return CommunityResource.banList({id:$stateParams.id}).$promise}]}};settings={name:"home.community.settings",url:"/settings",templateUrl:view_url+"/community/settings.html",controller:"communitySettingsCtrl"};rules={name:"home.community.rules",url:"/rules",templateUrl:view_url+"/community/rules.html",controller:"rulesCtrl",resolve:{communityRules:["CommunityResource","$stateParams",function(CommunityResource,$stateParams){return CommunityResource.rules({id:$stateParams.id}).$promise}]}};activityLog={name:"home.community.activityLog",url:"/activity-log",templateUrl:view_url+"/community/activity-log.html",resolve:{activityLog:["CommunityResource","$stateParams",function(CommunityResource,$stateParams){return CommunityResource.activityLog({id:$stateParams.id}).$promise}]},controller:["$scope","activityLog",function($scope,activityLog){return $scope.activityLog=activityLog}]};moderators={name:"home.community.moderators",url:"/moderators",views:{"right-rail@home":{resolve:{Moderators:["CommunityResource","$stateParams",function(CommunityResource,$stateParams){return CommunityResource.moderators({id:$stateParams.id}).$promise}]},templateUrl:view_url+"/community/moderators/moderatorList.html",controller:"moderatorListCtrl"}}};moderatorEdit={name:"home.community.moderators.edit",url:"/:user_id",views:{"@home.community":{resolve:{Moderator:["CommunityResource","$stateParams",function(CommunityResource,$stateParams){return CommunityResource.moderator({id:$stateParams.id,user_id:$stateParams.user_id}).$promise}]},templateUrl:view_url+"/community/moderators/editModerator.html",controller:"editModeratorCtrl"}}};moderationRequests={name:"home.community.moderationRequests",url:"/moderator_requests",views:{"right-rail@home":{resolve:{ModerationRequests:["CommunityResource","$stateParams",function(CommunityResource,$stateParams){return CommunityResource.moderationRequests({id:$stateParams.id}).$promise}]},templateUrl:view_url+"/community/moderators/moderationRequestList.html",controller:"moderationRequestListCtrl"}}};moderationRequestEdit={name:"home.community.moderationRequests.edit",url:"/:user_id",views:{"@home.community":{resolve:{UserStats:["UserResource","$stateParams",function(UserResource,$stateParams){return UserResource.stats({id:$stateParams.id}).$promise}]},templateUrl:view_url+"/community/moderators/editModerationRequest.html",controller:"editModerationRequestCtrl"}}};$stateProvider.state(home);$stateProvider.state(community);$stateProvider.state(communityQueue);$stateProvider.state(rules);$stateProvider.state(modwatch);$stateProvider.state(communityInbox);$stateProvider.state(policyGroups);$stateProvider.state(settings);$stateProvider.state(activityLog);$stateProvider.state(moderators);$stateProvider.state(moderatorEdit);$stateProvider.state(moderationRequests);return $stateProvider.state(moderationRequestEdit)}])}).call(this);
